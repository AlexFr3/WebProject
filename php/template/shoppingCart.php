<?php if (isset($templateParams["messaggiocarrello"])): ?>
  <div class="alert text-center <?php if (str_starts_with($templateParams["messaggiocarrello"], "L") || str_starts_with($templateParams["messaggiocarrello"], "G")) {
    ...
    <p class="fw-bold fs-2"><?php echo $templateParams["messaggiocarrello"]; ?></p>
  </div>
<?php endif; ?>
<section>
  <h1>Carrello</h1>
      ...
      <?php foreach ($templateParams["manga-in-carrello"] as $manga): ?>
        <li>
          <article>
            ...
            <input class="button-manga button-manga-quantity decrease-quantity" type="button" value="-" <?php if ($manga["Quantità_In_Carrello"] <= 1) {
              echo ("disabled");
            } ?> />
            <input type="hidden" value="<?php echo ($manga['idManga']) ?>" />
...
<section>
  <h1>Riepilogo</h1>
  <p class="lead fw-bold"> Numero Articoli: <?php echo ($templateParams["numero-articoli"]) ?><br />Prezzo Totale:
    <?php echo ($templateParams["prezzo-totale"]) ?>€</p>
